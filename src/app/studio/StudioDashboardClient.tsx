...
import { Upload, Music2, Disc3, TrendingUp, Users, Play, DollarSign, ChevronRight } from 'lucide-react'

interface Props {
  artist: { id: string; stage_name: string; genre: string }
  topTracks: any[]
  ...
  return (
    <div style={{ padding: '28px 24px', maxWidth: '900px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', marginBottom: '28px' }}>
        <Link href="/studio/upload" style={{ display: 'flex', alignItems: 'center', gap: '10px', background: '#2563eb', borderRadius: '12px', padding: '14px 16px', textDecoration: 'none', color: '#fff' }}>
          <Upload size={18} />
          <div>
            <p style={{ fontWeight: 700, fontSize: '14px', margin: 0 }}>Upload Music</p>
            <p style={{ fontSize: '11px', opacity: 0.7, margin: 0 }}>Track or album</p>
          </div>
        </Link>
      ...
      {topTracks.length > 0 && (
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
            <h2 style={{ color: '#fff', fontSize: '16px', fontWeight: 800, margin: 0 }}>Top Tracks</h2>
            <Link href="/studio/tracks" style={{ color: '#2563eb', fontSize: '13px', fontWeight: 600, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '2px' }}>
