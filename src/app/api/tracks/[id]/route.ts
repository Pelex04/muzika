...
export async function DELETE(
  ...
  // User client — for auth + ownership check (respects RLS)
  ...
  // From here: use service-role client so RLS never silently blocks the delete
  ...
  // Delete cover from storage — extract path from public URL
  if (track.cover_url) {
    try {
      const url = new URL(track.cover_url)
      const match = url.pathname.match(/\/public\/covers\/(.+)$/)
      if (match) {
        await admin.storage.from('covers').remove([match[1]])
      }
    } catch {
      // Non-fatal
    }
  ...
  // Delete the track record using service role (bypasses RLS)
